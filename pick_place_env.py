...
import time
from ur5_robotiq import UR5Robotiq85, UR5Robotiq140
...
class GoalObservationMixin:
    @staticmethod
    def dictobs2npobs(observation: Dict[str, np.ndarray], observation_space: spaces.Dict) -> np.ndarray:
        flat_obs = []
        for key, value in observation.items():
            space = observation_space[key]
            span = space.high - space.low
            normalized = (value - space.low) / span
            flat_obs.extend(normalized.reshape(-1))
        return np.asarray(flat_obs, dtype=np.float32)


class PickPlace_UR5Env(GoalObservationMixin):
...
class PickPlaceUR5eEnv(GoalObservationMixin):
    """
    Single-arm UR5e pick-and-place environment that reuses the same task API as the dual-arm variant.
    """
    ...
    def __init__(self, sim_params, robot_params, visual_sensor_params):
        ...
        if self.control_type == "joint" and self.gripper_enable:
            observation_bound_now = np.ones(shape=(self.arm_gripper.num_control_dofs,)) * math.pi
            observation_bound = np.concatenate([observation_bound_now, observation_bound_now])
        elif self.control_type == "joint" and not self.gripper_enable:
            observation_bound_now = np.ones(shape=(self.arm_gripper.arm_num_dofs,)) * math.pi
            observation_bound = np.concatenate([observation_bound_now, observation_bound_now])
        elif self.control_type == "end" and self.gripper_enable:
            observation_bound = np.concatenate(
                [
                    np.ones(shape=(3,)) * 2,
                    np.ones(shape=(3,)) * math.pi,
                    np.ones(shape=(3,)) * 2,
                    np.ones(shape=(3,)) * 3.2,
                    np.ones(shape=(self.arm_gripper.num_control_dofs - self.arm_gripper.arm_num_dofs,)) * math.pi,
                    np.ones(shape=(3,)) * math.pi,
                    np.ones(shape=(3,)) * 2,
                ]
            )
        ...
        action_dims = 3 if self.control_type == "end" else 6
        if self.gripper_enable:
            action_dims += 1
        self.action_space = spaces.Box(low=-1, high=1, shape=(action_dims,), dtype=np.float32)
    ...
    def reset(self, seed=None) -> Tuple[Dict[str, np.ndarray], Dict[str, Any]]:
        ...
        if self.is_train:
            ...
            while not initial_done:
                achieved_goal_initial, achieved_goal_initial_ang = self._sample_achieved_goal_initial()
                goal, goal_ang = self._sample_goal()
                initial_done = distance(achieved_goal_initial, goal) > self.distance_threshold
            self.achieved_goal_initial = achieved_goal_initial.copy()
            self.achieved_goal_initial_ang = achieved_goal_initial_ang
            self.goal = goal.copy()
            self.goal_ang = goal_ang
        else:
            self.achieved_goal_initial = np.array([0.72, 0.0, 0.04])
            self.achieved_goal_initial_ang = 0.0
            self.goal = np.array([0.75, 0.0, 0.04])
            self.goal_ang = 0.0

        if self.blockUid == -1:
            self.blockUid = self._pb.loadURDF(
                "./assets/urdf/cube_small_pick.urdf",
                self.achieved_goal_initial,
                self._pb.getQuaternionFromEuler([0, 0, self.achieved_goal_initial_ang]),
            )
            self.targetUid = self._pb.loadURDF(
                "./assets/urdf/cube_small_target_pick.urdf",
                self.goal,
                self._pb.getQuaternionFromEuler([0, 0, self.goal_ang]),
                useFixedBase=1,
            )
        else:
            self._pb.removeBody(self.blockUid)
    ...
    def _get_obs(self, robot_obs):
        ...
        return {"observation": total_obs, "achieved_goal": achieved_goal, "desired_goal": desired_goal}
    ...
    def _sample_achieved_goal_initial(self):
        achieved_goal_initial_xy = np.random.uniform(self.goal_range_low[:2], self.goal_range_high[:2])
        achieved_goal_initial = np.concatenate([achieved_goal_initial_xy, np.array(self.goal_range_low[-1]).reshape(1,) ])
        achieved_goal_initial_ang = np.random.uniform(-math.pi, math.pi)
        return achieved_goal_initial, achieved_goal_initial_ang
