...
class WGAN:
    def __init__(self, agent, state_dim, action_dim, hidden_dim, lr_d, device, epochs,c_lambda):
        self.device = device
        self.lr_d = lr_d
    ...
    def gradient_penalty(self, expert_sa, agent_sa, epsilon):
        ...
        gradient = torch.autograd.grad(inputs=mix_sa, outputs=mix_prob, grad_outputs=torch.ones_like(mix_prob))[0]
        gradient_norm = gradient.norm(2, dim=1)
        penalty = torch.mean((gradient_norm - 1) ** 2)*self.c_lambda
        return penalty
    def lr_decay(self, total_steps):
