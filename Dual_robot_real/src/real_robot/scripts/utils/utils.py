...
import rospkg
import inspect
from . import transformations, spalg
...
class PID:
    ...
    def update(self, error, dt=None):
        ...
        if self.dynamic_pid:
            kp = np.abs([self.Kp[i]/error[i] if error[i] != 0.0 else self.Kp[i] for i in range(len(error))])
            kp = np.clip(kp, self.Kp, self.Kp*self.max_gain_multiplier)
            kd = np.abs([self.Kd[i]*error[i] if error[i] != 0.0 else self.Kd[i] for i in range(len(error))])
