...
from ur5_task_space_pick_and_place import UR5PickAndPlaceEnv
import torch
...
obs, info = env.reset()
# print(obs)
cube_pose = env.get_cube_pose()
cube_position = np.array([cube_pose.position.x, cube_pose.position.y, cube_pose.position.z+0.02])
error = cube_position - obs['observation'][5:8]
# print(np.linalg.norm(cube_position - obs['observation'][5:8], axis=-1))
while np.linalg.norm(cube_position - obs['observation'][5:8], axis=-1)>env.distance_threshold:
    action = np.concatenate((np.array([0,0,0]),error,np.array([1,1])))
