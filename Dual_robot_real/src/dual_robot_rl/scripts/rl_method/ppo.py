...
def her_process(transition_dict,state_len,achieved_goal_len,distance_threshold,info):
    rewards = transition_dict['rewards']
    if info['is_success'] == True:
        return transition_dict,'is_goal'
    else:
        attach_matrix = np.array(transition_dict['states'])[:,:2]
        if np.any(attach_matrix):
            attached_step_idx = np.where(attach_matrix==1)[0][0]
            initial_achieved_goal = transition_dict['states'][attached_step_idx][state_len:state_len+achieved_goal_len]
            achived_goal_all = np.array(transition_dict['next_states'])[attached_step_idx:,state_len:state_len+achieved_goal_len]
            distances = np.linalg.norm(achived_goal_all-initial_achieved_goal,ord=2,axis=1)
            if np.all(distances<=distance_threshold):
                return transition_dict,'reach'
            else:
                is_goal_np = distances<=distance_threshold
                step_goal = np.random.choice(np.where(is_goal_np==False)[0])+attached_step_idx
