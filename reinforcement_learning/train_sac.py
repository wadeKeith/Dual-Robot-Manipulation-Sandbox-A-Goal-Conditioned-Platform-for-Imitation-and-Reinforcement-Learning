...
from sac_her import Agent_test, ReplayBuffer_Trajectory, SACContinuous, Trajectory
...
def evluation_policy(env, state_dim, action_dim, hidden_dim, device, model_num, model_dir: Path):
    ...
    model.load_state_dict(torch.load(model_dir / f"sac_her_ur5_pick_{model_num}.pkl"))
    model.eval()
    episode_return = 0
    state,_,_ = env.reset()
    done = False
...
buffer_snapshot_path = Path("sac_her_buffer_pickplace_all.pkl")
...
agent_num = 58
if load_agent:
    agent.load_state_dict(torch.load(model_dir / f"sac_her_ur5_pick_{agent_num}.pkl"))
...
for i in range(100):
    ...
    with tqdm(total=int(num_episodes), desc='Iteration %d' % i) as pbar:
        ...
        for i_episode in range(num_episodes):
            ...
            pbar.update(1)
    torch.save(agent.actor.state_dict(), model_dir / f"sac_her_ur5_pick_{i}.pkl")
