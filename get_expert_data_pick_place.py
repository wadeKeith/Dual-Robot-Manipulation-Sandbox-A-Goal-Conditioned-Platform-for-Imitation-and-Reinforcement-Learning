    ...
    while not done:
        ...
        if distance(achieved_goal,left_finger_pos)<=distance(achieved_goal,right_finger_pos):
            ...
            if distance(achieved_goal, finger_pos)>sim_params['distance_threshold'] and distance(achieved_goal,desired_goal)>sim_params['distance_threshold']:
                ...
                dz = np.clip((achieved_goal[2]-finger_pos[2]),-1,1)
                d_gripper = 1
                step_time = 0
                ...
                if step_time <10:
                    ...
                    d_gripper = -1
                step_time += 1
            action = np.array([dx,dy,dz,0,0,0,d_gripper,0])
        else:
            ...
            if distance(achieved_goal, finger_pos)>sim_params['distance_threshold'] and distance(achieved_goal,desired_goal)>sim_params['distance_threshold']:
                ...
                if step_time <10:
                    ...
                    d_gripper = -1
                step_time += 1
            action = np.array([0, 0, 0, dx,dy,dz,0,d_gripper])
...
file_name = "dual_robot_pickplace_"+str(savetime)+"_expert_data_WGCSL.pkl"
