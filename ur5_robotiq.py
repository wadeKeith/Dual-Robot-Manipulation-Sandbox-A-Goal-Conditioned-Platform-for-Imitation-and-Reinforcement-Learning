...
class UR5Robotiq140:
    """
    UR5 (single arm) with a Robotiq 140 gripper. Mirrors the dual-arm wrapper interface
    so environments and controllers can switch between embodiments with minimal churn.
    """
    ...
    def __init__(self, pb, robot_params, use_gui):
        ...
        self.action_scale = 0.2
        self.gripper_scale = 0.02
        self.tcp_link_name = robot_params.get("tcp_link_name", "ee_link")
        self.arm_rest_poses = robot_params["reset_arm_poses"]
        self.gripper_range = robot_params["reset_gripper_range"]
        self.load_urdf()
    ...
    def load_urdf(self):
        ...
        self.base_rpy = [0, 0, 0]
        self.base_orn = self._pb.getQuaternionFromEuler(self.base_rpy)
        asset_name = "./assets/urdf/ur5_robotiq_140.urdf"
        self.embodiment_id = self._pb.loadURDF(
            asset_name,
            self.base_pos,
            self.base_orn,
            useFixedBase=True,
            flags=self._pb.URDF_ENABLE_CACHED_GRAPHICS_SHAPES,
        )
        ...
        ) = self.create_link_joint_mappings(self.embodiment_id)
    ...
    def create_link_joint_mappings(self, urdf_id):
        ...
        # pull relevant info for controlling the robot
        joint_name_to_index = {}
        link_name_to_index = {}
        for i in range(num_joints):
            info = self._pb.getJointInfo(urdf_id, i)
    ...
    def open_gripper(self):
        current_length = math.sin(0.715 - self._pb.getJointState(self.embodiment_id, self.mimic_parent_id)[0]) * 0.1143 + 0.010
        action = (self.gripper_range[1] - current_length) / self.gripper_scale
        self.move_gripper(action)

    def close_gripper(self):
        current_length = math.sin(0.715 - self._pb.getJointState(self.embodiment_id, self.mimic_parent_id)[0]) * 0.1143 + 0.010
        action = (self.gripper_range[0] - current_length) / self.gripper_scale
        self.move_gripper(action)

    def move_gripper(self, action):
        current_length = math.sin(0.715 - self._pb.getJointState(self.embodiment_id, self.mimic_parent_id)[0]) * 0.1143 + 0.010
        target_length = np.clip(current_length + action * self.gripper_scale, *self.gripper_range)
        open_angle = 0.715 - math.asin((target_length - 0.010) / 0.1143)
        self._pb.setJointMotorControl2(
            self.embodiment_id,
            self.mimic_parent_id,
            self._pb.POSITION_CONTROL,
    ...
    def move_ee(self, action, control_method):
        ...
        assert control_method in ("joint", "end")
        if control_method == "end":
            ee_displacement = action * self.action_scale
            ee_position = np.array(self._pb.getLinkState(self.embodiment_id, self.tcp_link_id)[4])
            ee_orientation = np.array(self._pb.getLinkState(self.embodiment_id, self.tcp_link_id)[5])
            target_orientation = ee_orientation
            target_position = ee_position + ee_displacement
            target_position[2] = np.clip(target_position[2], 0.0, 1.12)
            joint_poses = np.array(
                self._pb.calculateInverseKinematics(
                    self.embodiment_id,
                    self.tcp_link_id,
                    target_position,
            ...
            arm_joint_ctrl = action * self.action_scale
            current_arm_joint_angles = np.array([self._pb.getJointState(self.embodiment_id, i)[0] for i in self.arm_controllable_joints])
            joint_poses = current_arm_joint_angles + arm_joint_ctrl
        for i, joint_id in enumerate(self.arm_controllable_joints):
            self._pb.setJointMotorControl2(
    ...
    def setup_ur5_info(self):
        ...
        for i in range(self._pb.getNumJoints(self.embodiment_id)):
            ...
            if controllable:
                self.control_joint_ids.append(jointID)
                self._pb.setJointMotorControl2(
                    self.embodiment_id, jointID, self._pb.VELOCITY_CONTROL, targetVelocity=0, force=0
                )
            info = jointInfo(
                jointID,
                jointName,
    ...
    def setup_gripper_info(self):
        ...
        for joint_id, multiplier in self.mimic_child_multiplier.items():
            constraint = self._pb.createConstraint(
                self.embodiment_id,
                self.mimic_parent_id,
                self.embodiment_id,
                joint_id,
            ...
            self._pb.changeConstraint(constraint, gearRatio=-multiplier, maxForce=100, erp=1)
    ...
    def get_joint_obs(self, control_type, gripper_enable) -> np.ndarray:
        ...
        if control_type == "joint":
            ...
            ee_info = self._pb.getLinkState(self.embodiment_id, self.tcp_link_id, computeLinkVelocity=1)
            ee_pos = list(ee_info[4])
            ee_orn = list(self._pb.getEulerFromQuaternion(np.array(self._pb.getLinkState(self.embodiment_id, self.tcp_link_id)[5])))
            ee_linear_velocity = list(ee_info[6])
            ee_angular_velocity = list(ee_info[7])
            finger_left = self._pb.getLinkState(self.embodiment_id, self.left_finger_pad_id, computeLinkVelocity=1)
            finger_right = self._pb.getLinkState(self.embodiment_id, self.right_finger_pad_id, computeLinkVelocity=1)
            finger_pos = list((np.array(finger_left[4]) + np.array(finger_right[4])) / 2)
            finger_linear_velocity = list((np.array(finger_left[6]) + np.array(finger_right[6])) / 2)
            finger_angular_velocity = list((np.array(finger_left[7]) + np.array(finger_right[7])) / 2)
            arm_obs = (
                finger_pos
                + ee_pos
                + ee_orn
                + finger_linear_velocity
                + finger_angular_velocity
                + ee_linear_velocity
                + ee_angular_velocity
            )
            positions = arm_obs
    ...
    def get_arm_joints_in_bounds(self, target_joint_angles: np.ndarray) -> np.ndarray:
        bounded_joint_angles = np.clip(target_joint_angles, self.arm_lower_limits, self.arm_upper_limits)
        return bounded_joint_angles

    @staticmethod
    def get_joint_pos_range(min_pos: np.ndarray, max_pos: np.ndarray):
        assert min_pos.size == max_pos.size
        joint_state = namedtuple("joint_state", ["min", "max"])
        return joint_state(min_pos, max_pos)
